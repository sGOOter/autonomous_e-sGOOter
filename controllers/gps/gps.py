...
# Open a file for GPS logging
gps_log_file = open("/Users/apple/Desktop/robotic_project/sGOOter_project/controllers/gps/park.txt", "a")  # Append mode

gps_record_interval = 1000  # milliseconds
last_gps_record_time = 0
...
while robot.step(timestep) != -1:
    current_time = robot.getTime() * 1000  # Get current time in milliseconds

    if current_time - last_gps_record_time >= gps_record_interval:
        coords = gps.getValues()
        print(coords)
        gps_log_file.write(f"{current_time}, {coords[0]:.2f}, {coords[1]:.2f}, {coords[2]:.2f}\n")
        last_gps_record_time = current_time
