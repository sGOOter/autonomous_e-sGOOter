import cv2
import time
import pickle
import numpy as np
from controller import Supervisor

robot = Supervisor()
timestep = int(robot.getBasicTimeStep())

robotNode = robot.getSelf()
preview = 0 # Mask Preview 1


Kp = 0.01
Ki = 0.02
Kd = 0.0001

P = 0
I = 0
D = 0
oldP = 0
PID = 0

# Bicycle speed and handlebar settings
maxS = 4 # max speed
minS = 0 # min speed
bcyS = 2
acceleration = 2  # Acceleration factor
deceleration = 0.5  # Deceleration factor

hMax = 0.1 # rads (11°) Max
hndB = 0 # center initially
maxV = 0 # Max Velocity

# Motor and handlebar controls
whemotor = robot.getDevice('motor::crank')
whemotor.setPosition(float('inf'))
whemotor.setVelocity(bcyS)  # Set initial velocity to move forward

hndmotor = robot.getDevice('handlebars motor')
hndmotor.setPosition(0)

# Keyboard setup
robot.keyboard.enable(timestep)
robot.keyboard = robot.getKeyboard()

# Camera and display setup
camera = robot.getDevice('camera')
camera.enable(timestep*4)

display = robot.getDevice('display')
display.attachCamera(camera)
display.setColor(0x00FF00)
display.setFont('Verdana', 16, True)

if preview == 1:
    cv2.startWindowThread()

last_error = 0  # Initialize the last error as a global variable outside the function


svm = None
with open('./model/intention.pkl', 'rb') as f:
    svm = pickle.load(f)

def get_intention():
    import speech_recognition as sr

    recognizer = sr.Recognizer()
    microphone = sr.Microphone()

    print("listening...")
    with microphone as source:
        recognizer.adjust_for_ambient_noise(source)
        audio_data = recognizer.listen(source)

    try:
        text = recognizer.recognize_google(audio_data, language='en-US') # 识别英文
        intention = svm.predict(text)
        print("recognized:", text, intention)
        return intention
    except Exception as e:
        print("recognize intention: ", e)
    
    return None

import threading
import speech_recognition as sr

# 创建一个Recognizer对象
recognizer = sr.Recognizer()

# 使用系统默认的麦克风
microphone = sr.Microphone()

def listen_microphone():
    global recognizer, microphone
    
    while True:
        print("recording start")
        with microphone as source:
            recognizer.adjust_for_ambient_noise(source)
            try:
                audio_data = recognizer.listen(source, timeout=2) 
            except sr.WaitTimeoutError:
                print("recording timeout")
                audio_data = None 
        
        threading.Thread(target=process_audio, args=(audio_data,), daemon=True).start()

command = None

def listen_intention():
    global command
    while True:
        time.sleep(1)

        import intention
        inten = intention.read_file()
        if inten != "":
            command = inten
            print("listen intention successfully: ", command)


def process_audio(audio_data):
    global recognizer
    global command
    
    if not audio_data:
        return

    try:
        text = recognizer.recognize_google(audio_data, language='en-US') # 识别英文
        intention = svm.predict([text])
        if len(intention)>0:
            command = intention[0]

        print("recognize intention successfully: ", text, "->", command)
    except Exception as e:
        print("recognize intention:", e)

def get_error():
    img = np.frombuffer(camera.getImage(), dtype=np.uint8).reshape((camera.getHeight(), camera.getWidth(), 4))
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    lower_white, upper_white = np.array([0, 0, 200], dtype=np.uint8), np.array([180, 50, 255], dtype=np.uint8)
    mask = cv2.inRange(hsv, lower_white, upper_white)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    if contours:
        largest_contour = max(contours, key=cv2.contourArea)
        M = cv2.moments(largest_contour)
        if M["m00"] != 0:
            cx = int(M["m10"] / M["m00"])
            center_x = camera.getWidth() // 2
            return (cx - center_x) * 0.03
    return 0


def handle_voice():
    global command
    global bcyS, hndB, manual_control

    key = command
    if key is None:
        return False
    
    if key == 'accelerate':  # Up
        bcyS = min(bcyS+1, maxS)
    elif key == 'decelerate':  # Down
        bcyS = max(bcyS-1, minS)
    elif key == 'stop':
        bcyS = 0  # Stop

    if key == 'turn left':  # Left
        hndB = hMax
    elif key == 'turn right':  # Right
        hndB = -hMax
    else:
        hndB = 0  # Center

    whemotor.setVelocity(bcyS)
    hndmotor.setPosition(hndB)

    command = None

    print("[handle voice]executing intention: ", key, [bcyS, hndB])
    return True 


def handle_keys():
    global bcyS, hndB, manual_control

    key = robot.keyboard.getKey()
    if key == -1:
        return False

    if key == ord('M'):
        manual_control = not manual_control
        print("Switched to", "manual" if manual_control else "automatic", "control")

    if key == 315:  # Up
        key = 'accelerate'
        bcyS = min(bcyS+1, maxS)
    elif key == 317:  # Down
        key = 'decelerate'
        bcyS = max(bcyS-1, minS)
    elif key == ord('S'):
        key = 'stop'
        bcyS = 0  # Stop

    if key == 314:  # Left
        key = 'turn left'
        hndB = hMax
    elif key == 316:  # Right
        key = 'turn right'
        hndB = -hMax
    else:
        hndB = 0  # Center

    whemotor.setVelocity(bcyS)
    hndmotor.setPosition(hndB)

    print("[handle key]executing intention: ", key, [bcyS, hndB])
    return True
    
    
def hms(sec):
    # Convert seconds to hours:minutes:seconds
    h = sec // 3600
    m = (sec % 3600) // 60
    s = (sec % 3600) % 60
    return f'{h:02d}:{m:02d}:{s:02d}'

def printStatus():
    global maxV
    velo = robotNode.getVelocity()
    velocity = np.sqrt(np.sum(np.square(velo[:3]))) * 3.6  # Convert m/s to km/h
    if velocity > maxV:
        maxV = (velocity + maxV) / 2

    timer = int(robot.getTime())
    strP = hms(timer)
    
    robot.setLabel(0, f'Robot: {robot.getName()} Speed: {velocity:.2f} km/h Max: {maxV:.2f} km/h', 0, 0.05, 0.06, 0x000000, 0, 'Lucida Console')



STEERING_RESET_TOLERANCE = 0.0015

manual_control = False
key_pressed = False
toggle_pressed = False

from datetime import datetime


threading.Thread(target=listen_intention, daemon=True).start()
while robot.step(timestep) != -1:
    if handle_keys():
        print('[manu]', datetime.now().strftime("%H:%M:%S"))
    elif handle_voice():
        print('[voice]', datetime.now().strftime("%H:%M:%S"))
    else:
        # Autonomous control based on line detection
        current_error = get_error()
        P = current_error
        I += (2 / 3) * P * (timestep / 1000)
        D = 0.5 * (P - oldP) / (timestep / 1000)
        PID = Kp * P + Ki * I + Kd * D
        oldP = P
    
        if abs(PID) < STEERING_RESET_TOLERANCE:
            hndB = 0
        else:
            max_handlebar_angle = np.clip(abs(PID), 0, -hMax)
            hndB = np.sign(PID) * max_handlebar_angle * 0.65
    
        whemotor.setVelocity(max(minS, min(bcyS, maxS)))
        hndmotor.setPosition(hndB)